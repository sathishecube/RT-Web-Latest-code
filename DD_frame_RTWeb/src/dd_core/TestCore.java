...
public class TestCore {
	...
	@Test (priority=42)
	public static void ReeferRouteHistory() throws Exception
	{
		if(input[42][0].equalsIgnoreCase("ReeferRouteHistory") && input[42][1].equalsIgnoreCase("Y"))
		{
		ExtentTest test =reports.startTest("ReeferRouteHistory");
		test.appendChild(test);
		...
		data1 =ReeferRouteHistory.RouteHistory(data1, rc, "ReeferRouteHistory",test,scase,ecase);
		System.out.println("rc"+rc);
		rc=rc+data1.size();
		System.out.println("data size"+data1.size());
		data.putAll(data1);
